---
sidebar_position: 15
title: Kuksa-to-LIVI Telemetry Bridge
---

# Kuksa-to-LIVI Telemetry Bridge

The **Kuksa-to-LIVI Telemetry Bridge** (`kuksa-livi-bridge`) is an Ankaios workload on the **Raspberry Pi 5** that feeds live VSS values from the Kuksa Databroker into the [LIVI](https://github.com/f-io/LIVI) IVI head unit running on the [Raspberry Pi 4](./device-ivi-livi).

It is the symmetric counterpart of the [MQTT-to-Kuksa gRPC Bridge](./bridge-mqtt-grpc): where that bridge ingests sensor data into the databroker, the LIVI bridge **forwards** databroker state to the dashboard.
...
- [IVI Head Unit (LIVI)](./device-ivi-livi.md) — the Pi 4 device that consumes the telemetry stream.
- [MQTT-to-Kuksa gRPC Bridge](./bridge-mqtt-grpc.md) — sister bridge in the opposite direction (Mosquitto → Kuksa).
- [gRPC-to-Gamecontroller Bridge](./bridge-gamecontroller.md) — third consumer of the central databroker.
